...
ArrayBasedQueue &ArrayBasedQueue::operator=(ArrayBasedQueue &&arrayBasedQueue) {
    ...
    swap(_size, arrayBasedQueue._size);
    swap(_front, arrayBasedQueue._front);
    swap(_rear, arrayBasedQueue._rear);
    swap(_array, arrayBasedQueue._array);
    return *this;
}
...
ArrayBasedQueue::ArrayBasedQueue(size_t size) : _size(size), _front(0), _rear(0) {
    _array = new int[_size];
}
...
size_t ArrayBasedQueue::size() const {
    return _size;
}
